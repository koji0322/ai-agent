...
VBA で例えると、マクロ付きブックを共有フォルダに置いて同僚に使ってもらい、フィードバックを受けて改善する段階に相当します。
</details>
...
VBA で例えると、マクロ付きブックに添える「使い方」シートに相当します。ただし README はマークダウンで書くので、見出し・箇条書き・コードブロックなど表現力が豊かです。
</details>
...
VBE の「ツール → 参照設定」で外部ライブラリ（Microsoft Scripting Runtime など）を指定していたのと同じです。参照設定が抜けていると「ユーザー定義型は定義されていません」エラーになりますが、Python でも同様に、必要なパッケージを明示しておかないと他の環境で動きません。
</details>
...
VBA で例えると、マクロ付きブックを共有フォルダに置くのがデプロイに相当します。Web アプリでは、サーバーにアプリを置きます。
</details>
...
VBA の `Workbook_Open` で自動チェックを走らせていたのと似ています。ただしこちらは Git push をトリガーに、GitHub 上でテストやデプロイが自動実行されます。
</details>
