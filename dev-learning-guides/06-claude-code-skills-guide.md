...
- **想定読者**: [Claude Code 公開・運用ガイド](05-claude-code-deploy-guide.md) を読み終え、プロジェクトの開発・公開・運用を一通り経験した
...
- **関連ガイド**:
  ...
  - [Claude Code 公開・運用ガイド](05-claude-code-deploy-guide.md) — 公開・運用（レベル 5）
  - [Claude Code マルチエージェントガイド](07-claude-code-multi-agent-guide.md) — マルチエージェント（次のステップ）
...
> VBA で例えると、個人用マクロを整理して汎用アドイン（`.xlam`）にまとめ、社内に配布する段階。
...
> カスタムコマンドが「よく使うマクロをボタンに登録する」なら、スキルは「専門家の知識をマニュアルにまとめて配布する」イメージ。
...
> VBA で例えると: レベル 1 は「マクロの名前一覧」、レベル 2 は「マクロのコード本体」、レベル 3 は「マクロが参照する外部データ」。全部を一度にメモリに載せず、必要なときだけ読み込む。
...
> VBA ユーザーにとって特に価値が高いのが Excel スキル。Claude Code から Excel ファイルを生成・操作できる。
...
> VBA の対比: `.xlam`（Excel アドイン）ファイル。複数のモジュール・フォーム・参照設定を 1 つにまとめて配布する仕組みに相当する。
