...
| プロジェクトルートの `CLAUDE.md` | プロジェクト固有の情報 | 「Python 3.13 を使用」「テストは pytest で実行」 |
...
| `PostToolUseFailure` | ツール実行が失敗したとき | エラーログの記録、リトライ処理 |
| `PermissionRequest` | 権限の確認が発生したとき | カスタムの承認フロー |
| `SubagentStart` | サブエージェント起動時 | サブエージェントの監視 |
| `SubagentStop` | サブエージェント終了時 | 結果の集約・ログ |
| `TeammateIdle` | チームメイトが待機状態になったとき | Agent Teams で活用 |
| `TaskCompleted` | タスクが完了したとき | 品質ゲート、テスト自動実行 |
| `PreCompact` | コンテキスト圧縮前 | 重要な情報の保存 |
| `ConfigChange` | 設定が変更されたとき | 設定変更のログ記録 |

### ハンドラーのタイプ

フックで実行する処理には 3 つのタイプがある。

| タイプ | 動作 | 用途 |
|-------|------|------|
| `command` | シェルコマンドを実行 | フォーマット、通知、ログ記録 |
| `prompt` | LLM にプロンプトを送って判断させる | 危険な操作の判定、品質チェック |
| `agent` | 別のエージェントを起動して処理 | 複雑な分析・レビュー |

### 設定方法

`.claude/settings.json`（または `~/.claude/settings.json`）の `hooks` セクションに記述する。

> **ヒント**: `/hooks` コマンドを使うと、対話メニューでフックの設定・編集ができる。JSON を手書きするより簡単。
...
`command` タイプに加えて、LLM に判断を委ねる `prompt` タイプも使える。
...
**特定の役割を持った専門家の Claude Code** を定義ファイルとして保存できる仕組み。`.claude/agents/` フォルダにマークダウンファイルを置く。

> カスタムコマンドが「よく使う指示のショートカット」なら、カスタムエージェントは「専門家を雇う」イメージ。

### 作り方

`.claude/agents/` フォルダにマークダウンファイルを作成する。
...
```
/agents:reviewer src/main.py をレビューして
```

### ユーザーレベルのエージェント

`~/.claude/agents/` に置くと、全プロジェクトで使えるエージェントになる。
...
> 「Claude Code が動かない」「何かおかしい」というときに、まず `claude doctor` を実行すると原因がわかることが多い。

### Git Worktree モード

```bash
# 隔離された Git worktree で作業を実行
claude --worktree "この機能を実装して"
```

`--worktree` をつけると、現在のブランチに影響を与えずに別の作業ツリーで作業が行われる。複数のタスクを並行して進めるときに安全。
...
会話をリセットせず、これまでの会話を要約して圧縮する。`/clear` と違い、文脈を完全に失わずにコンテキストウィンドウの空きを増やせる。
...
現在の会話のコピーを作り、別の方向に進めたいときに使う。「ここまでの会話を残したまま、別のアプローチを試す」ときに便利。
