...
- **想定読者**: [Claude Code アプリケーション開発ガイド](04-claude-code-app-dev-guide.md) を読み終え、Python スクリプトや簡単な Web アプリを作れる
...
- **関連ガイド**:
  ...
  - [Claude Code アプリケーション開発ガイド](04-claude-code-app-dev-guide.md) — アプリ開発（レベル 4）
  - [Claude Code スキル・プラグインガイド](06-claude-code-skills-guide.md) — スキル・プラグイン（次のステップ）

> レベル 4 までは **ローカル環境（自分の Mac の中）** で完結していた。
> このガイドでは、作ったものを **他の人が使える形にして公開し、維持・改善を続ける** ところまでをカバーする。
> VBA で例えると、マクロ付きブックを共有フォルダに置いて同僚に使ってもらい、フィードバックを受けて改善する段階。
...
> VBA で例えると、マクロ付きブックに添える「使い方」シートに相当する。ただし README はマークダウンで書くので、見出し・箇条書き・コードブロックなど表現力が豊か。
...
> VBA の対比: VBE の「ツール → 参照設定」で外部ライブラリ（Microsoft Scripting Runtime など）を指定していたのと同じ。参照設定が抜けていると「ユーザー定義型は定義されていません」エラーになる。Python でも同様に、必要なパッケージを明示しておかないと他の環境で動かない。
...
> VBA の対比: マクロ付きブックを共有フォルダに置くのがデプロイ。Web アプリでは、サーバーにアプリを置く。
...
> VBA の対比: `Workbook_Open` で自動チェックを走らせていたのと似ている。ただしこちらは Git push をトリガーに、GitHub 上でテストやデプロイが自動実行される。
