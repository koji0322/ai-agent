...
VBE は Excel 専用のコードエディタで、マクロを書くための環境として Excel の中に組み込まれています。
VS Code はあらゆる言語に対応する汎用コードエディタです。
VBE が「Excel 付属の専用工具」なら、VS Code は「何にでも使える万能工具箱」といえます。
...
VBE との対応関係:
- **アクティビティバー**: VBE にはない機能
- **サイドバー**: VBE のプロジェクトエクスプローラーに相当
- **エディタ領域**: VBE のコードウィンドウに相当
- **ターミナルパネル**: VBE のイミディエイトウィンドウに似ているが、本格的なターミナル
- **ステータスバー**: VBE の下部ステータス表示に相当
...
VBE のイミディエイトウィンドウ（`Ctrl + G` で開く）は簡単なコード実行用でしたが、VS Code の内蔵ターミナルは本格的なターミナルそのものです。外部のターミナルアプリと同じことがすべてできます。
...
VBA でいう「アドイン」に近い概念です。Excel 単体にはない機能をアドインで追加するように、VS Code も拡張機能で機能を追加できます。
