...
| 観点 | Excel/VBA | Python（pandas） |
|------|-----------|-------------------|
| データ量 | 数万行が限界 | 数百万行でも処理可能 |
| 再現性 | マクロを書けば再現可能だが共有しにくい | スクリプトをそのまま共有・再実行できる |
| 自動化 | VBA マクロで自動化 | スクリプト実行で完全自動化 |
| 拡張性 | Excel の機能に依存 | 豊富なライブラリで機能拡張 |
| バージョン管理 | 困難 | Git で管理可能 |
| コスト | Microsoft 365 が必要 | すべて無料 |
