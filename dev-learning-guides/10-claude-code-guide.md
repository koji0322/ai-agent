...
VBE（Visual Basic Editor）では、プロジェクトエクスプローラーでモジュールを選び、コードを書いて実行ボタンを押す。ターミナルでは同様の操作をコマンド入力で行う。やることは似ているが、操作方法が異なる。
...
VBA では「特定のブックを開いてからマクロを実行する」のと同じ。Claude Code も作業対象のフォルダに移動してから起動することで、そのフォルダ内のファイルを扱える。
...
VBA の「標準モジュールの先頭にプロジェクトの説明コメントを書く」のに似ているが、CLAUDE.md は Claude Code 専用の設定ファイルとして、起動するたびに自動で読み込まれる点が異なる。
