...
| コマンド | VBA で例えると |
|---------|---------------|
| `git init` | 新しいブックを作成する |
| `git add ファイル名` | 保存したいシートを選ぶ |
| `git commit -m "説明"` | `Ctrl + S` で保存する |
</details>
...
VBA では 1 つの標準モジュールに数百行のコードを書くことが多いが、実際のプロジェクトでは機能ごとにファイルを分ける。
</details>
